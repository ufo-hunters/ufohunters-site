...
class SightingsController < ApplicationController
  ...
  def ufosearchresults
    if verify_recaptcha
      ...
      @ufo_list = Report.where(coord: { '$nearSphere' => @coords, '$maxDistance' => distance.fdiv(6371) })
                        .and(status: 1)
                        .and(sighted_at: { '$gte' => startdate })
                        .and(sighted_at: { '$lte' => enddate })
                        .desc(:sighted_at).limit(200).entries
      ...
      respond_to do |format|
        ...
        format.json { render json: @ufo_list, status: :unprocessable_content }
      end
    end
  end
  ...
  def country
    ...
    if type == 'Polygon'
      @ufo_list = Rails.cache.fetch("sightings/country/#{country_code}/polygon", expires_in: 1.day) do
        Report.where(coord: { '$geoWithin' => { '$polygon' => coordinates[0] } })
              .and(status: 1).without(:email).order_by(sighted_at: :desc).limit(100).entries
      end
    else
      coordinates.each_with_index do |data, index|
        @ufo_list = if index.zero?
                      Rails.cache.fetch("sightings/country/#{country_code}/polygon/#{index}", expires_in: 1.day) do
