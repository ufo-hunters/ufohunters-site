...
class ApplicationHelperTest < ActionView::TestCase
  ...
  setup do
    @dates_to_format = %w[20130520 20130101 19010831]
  ...
  test 'should format string date for microdata' do
    expected_responses = %w[2013-05-20 2013-01-01 1901-08-31]
  ...
  test 'should not detect no youtube urls' do
    @no_youtube_urls.each do |link|
      assert_not youtube_link?(link), 'Should detect only youtube links'
    end
  end

  test 'should extract video id' do
    expected_responses = %w[rhKgzczo8UM xjFHveMSoYQ]

    actual_responses = @youtube_urls.map do |link|
  ...
  def assert_string_date_format(method_name, dates_to_format, expected_responses)
    actual_responses = dates_to_format.map do |date_to_format|
      ApplicationHelperTest.send(method_name, date_to_format)
    end
