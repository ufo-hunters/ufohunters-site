...
class ArticlesHelperTest < ActionView::TestCase
  ...
  setup do
    @articles = [
      ...
      Article.new(title: ".Title?of?'article' with lots of 'chars'&to&remove/replace.",
                  published_date: '20130329')
    ]

    @dates_to_format = %w[20011201 20140930 19000221]
  end
  ...
  test 'should generate an article friendly title' do
    ...
    actual_responses = @articles.map do |article|
      ArticlesHelper.friendly_title(article)
    end
  ...
  test 'should format string date' do
    expected_responses = %w[2001-12-01 2014-09-30 1900-02-21]

    actual_responses = @dates_to_format.map do |date_to_format|
      ArticlesHelper.format_date(date_to_format)
    end
